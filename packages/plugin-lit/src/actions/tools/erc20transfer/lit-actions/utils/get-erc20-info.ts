...
export async function getTokenInfo(
  ...
  elizaLogger.log('Getting token info for:', tokenIn);

  // Validate token address
  try {
    elizaLogger.log('Validating token address...');
  ...
  elizaLogger.log('Checking if contract exists...');
  ...
  elizaLogger.log('Creating token contract instance...');
  ...
  elizaLogger.log('Fetching token decimals and balance...');
  try {
    const decimals = await tokenContract.decimals();
    elizaLogger.log('Token decimals:', decimals);

    const amount = ethers.utils.parseUnits(params.amountIn, decimals);
    elizaLogger.log('Amount to send:', amount.toString());

    const pkpBalance = await tokenContract.balanceOf(pkpEthAddress);
    elizaLogger.log('PKP balance:', pkpBalance.toString());
